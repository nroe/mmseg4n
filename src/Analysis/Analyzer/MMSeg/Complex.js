    ...
    seg: function(sen)
    {
        ...
        if (! sen.isSegFinish()) {
            ...
            for (var i=0; i<this.otherFilter.length; i++) {
                var filter = this.otherFilter[i];
                
                if (chunks.length > 1) {
                    filter.reset();
                    filter.addChunks(chunks);
                    chunks = filter.remian();
                } else {
                    break;
                }
            }
    ...
    createChunk: function(sen, tailLen, offsets, cns)
    {
        ...
        for (var i=0; i< 3; i++) {
            if (offsets[i] < chs.length) {
                ...
                // console.log("token:" + token.getTermText() + " startOffset:" + token.getStartOffset() + " endOffset:" + token.getEndOffset() + " type:" + token.getType());
